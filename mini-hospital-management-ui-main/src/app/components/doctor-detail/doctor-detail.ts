...
export class DoctorDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.doctorService.getDoctorById(id).subscribe({
      next: (doctor: any) => {
        ...
        this.profileImgUrl = `https://hospitalmanagementapi-jybs.onrender.com/api/Doctor/${doctor.doctorId}/profile-image?${timestamp}`;
        this.loading = false;
      },
      error: (err) => {
  ...
  getProfileImageUrl(): string {
    return `https://hospitalmanagementapi-jybs.onrender.com/api/Doctor/${this.doctorId}/profile-image?ts=${Date.now()}`;
  }
